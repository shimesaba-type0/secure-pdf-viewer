...
def migrate_password_to_passphrase(db):
    ...
    current_setting = db.execute(
        "SELECT value FROM settings WHERE key = ?", ("shared_password",)
    ).fetchone()
...
def run_migration_001(db):
    ...
    try:
        ...
        """,
            (
                "001_password_to_passphrase",
                "Migrate shared_password to shared_passphrase with validation",
            ),
        )
...
def run_migration_002(db):
    ...
    try:
        ...
        """,
            (
                "002_security_event_logging",
                "Add security event logging tables and indexes",
            ),
        )
...
def run_migration_003(db):
    ...
    try:
        ...
        """,
            (
                "003_pdf_table_columns",
                "Add published_date and unpublished_date columns to pdf_files table",
            ),
        )
...
def run_migration_004(db):
    """マイグレーション004: 管理者ロール体系とセッション管理強化"""
    print("Running Migration 004: Admin Role System and Session Management Enhancement")

    try:
        # 既にトランザクション内のため、BEGIN TRANSACTIONは不要

        # admin_usersテーブルにroleカラムを追加
        try:
            ...
            if "duplicate column name" not in str(e).lower():
                raise
            print("role column already exists in admin_users")

        # 既存の最初の管理者をsuper_adminに設定
        db.execute(
            """
            UPDATE admin_users
            SET role = "super_admin"
            WHERE id = (SELECT MIN(id) FROM admin_users WHERE is_active = TRUE)
        """
        )
        print("Set first admin as super_admin")

        # 管理者セッション制限設定を追加
        settings_to_add = [
            (
                "super_admin_unlimited_sessions",
                "true",
                "boolean",
                "スーパー管理者の無制限セッション許可",
                "security",
            ...
            (
                "session_rotation_max_age_hours",
                "24",
                "integer",
                "セッション強制ローテーション時間（時間）",
                "security",
            ...
            (
                "session_rotation_alert_threshold",
                "5",
                "integer",
                "セッションローテーション警告閾値",
                "security",
            ...
            (
                "session_rotation_lock_threshold",
                "10",
                "integer",
                "セッションローテーションロック閾値",
                "security",
        ...
        print("Added admin session management settings")
        ...
        print("Created admin_session_events table")

        # インデックス作成
        session_event_indexes = [
            "CREATE INDEX IF NOT EXISTS idx_admin_session_events_admin_email ON admin_session_events(admin_email)",
            "CREATE INDEX IF NOT EXISTS idx_admin_session_events_created_at ON admin_session_events(created_at)",
            "CREATE INDEX IF NOT EXISTS idx_admin_session_events_event_type ON admin_session_events(event_type)",
        ]

        for index_sql in session_event_indexes:
            db.execute(index_sql)
        print("Created admin session event indexes")
        ...
        """,
            (
                "004_admin_role_session_management",
                "Add role system to admin_users and enhanced session management settings",
            ),
        )

        # 既にコンテキストマネージャー内のため、COMMITは不要
...
def run_all_migrations(db):
    ...
    available_migrations = [
        ...
        ("004_admin_role_session_management", run_migration_004),
    ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 結果確認
        result = conn.execute(
            "SELECT key, value FROM settings WHERE key = ?", ("shared_passphrase",)
        ).fetchone()
        if result:
