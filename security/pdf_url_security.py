...
class PDFURLSecurity:
    ...
    def generate_signed_url(
        self, filename, session_id, expiry_hours=72, one_time=False, conn=None
    ):
        """
        署名付きPDF配信URLを生成する
        ...
        try:
            ...
            if conn:
                try:
                    configured_expiry = get_setting(
                        conn, "pdf_url_expiry_hours", expiry_hours
                    )
            ...
            sign_string = (
                f"{filename}:{expiry_timestamp}:{session_id}:{str(one_time).lower()}"
            )
            ...
            token = (
                base64.urlsafe_b64encode(query_string.encode("utf-8"))
                .decode("utf-8")
                .rstrip("=")
            )
    ...
    def create_pdf_access_url(self, pdf_file_info, session_id, conn=None):
        ...
        result = self.generate_signed_url(
            filename=pdf_file_info["stored_name"], session_id=session_id, conn=conn
        )
